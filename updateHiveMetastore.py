...
def mysql_hive_metastore_creation():
	try:
		...
		connection = MySQLdb.connect(host=HIVE_DB_HOST, port=int(HIVE_DB_PORT), user="root", passwd="root123")
		...
		if len(data)==0:
			...
			utils.run_command(command)
			command = "mysql -uroot -proot123 -h "+HIVE_DB_HOST+" -P "+HIVE_DB_PORT+" --execute=\"GRANT SELECT,INSERT,UPDATE,DELETE,LOCK TABLES,EXECUTE ON "+HIVE_DB_NAME+".* TO 'hive'@'"+ HIVE_DB_HOST +"' IDENTIFIED BY '" + HIVE_DB_PWD+ "';\""
			utils.run_command(command)
			# grant hive user to access database from other hosts
			command = "mysql -uroot -proot123 -h "+HIVE_DB_HOST+" -P "+HIVE_DB_PORT+" --execute=\"GRANT SELECT,INSERT,UPDATE,DELETE,LOCK TABLES,EXECUTE ON "+HIVE_DB_NAME+".* TO 'hive'@'%' IDENTIFIED BY '" + HIVE_DB_PWD+ "';\""
			utils.run_command(command)
			command = "mysql -uroot -proot123 -h "+HIVE_DB_HOST+" -P "+HIVE_DB_PORT+" --execute=\"GRANT SELECT,INSERT,UPDATE,DELETE,LOCK TABLES,EXECUTE ON "+HIVE_DB_NAME+".* TO 'hive'@'localhost' IDENTIFIED BY '" + HIVE_DB_PWD+ "';\""
			utils.run_command(command)
			
			command = "mysql -uroot -proot123 -h "+HIVE_DB_HOST+" -P "+HIVE_DB_PORT+" --execute='FLUSH PRIVILEGES;'"
			utils.run_command(command)
